document.addEventListener('DOMContentLoaded', function () {
        ...
        paginationContainer.addEventListener('click', async (e) => {
            if (e.target.classList.contains('page-link')) {
                ...
                if (page) {
                    ...
                    window.history.pushState({}, '', `/users/blocked?${queryParams.toString()}`);
    ...
    async function loadUsers(queryParams) {
        try {
            showLoading();
            const response = await fetch(`/users/blocked?${queryParams.toString()}`, {
                headers: {
    ...
    function updateUserTable(users) {
        ...
        tbody.innerHTML = users.map(user => {
            return `
                ...
                <!-- Modal Unblock -->
                <div class="modal" id="unblockModal-${user.id}" tabindex="-1" aria-labelledby="unblockModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="unblockModalLabel">Confirm Unban</h5>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-dark button-unblock-modal" data-id="${user.id}" data-bs-dismiss="modal">
                                    Unban
                                </button>
                            </div>
                        </div>
    ...
    // Handle unblock modal functionality
    document.querySelectorAll('.button-unblock-modal').forEach(button => {
        button.addEventListener('click', async (e) => {
            e.preventDefault();
            showLoading();
            ...
            try {
                ...
                if (response.ok) {
                    hideLoading();
                    showAlert('success', 'Success', 'User unblocked successfully');
                    window.location.reload();
                } else {
                    const result = await response.json();
                    hideLoading();
                    showAlert('error', 'Error', result.message || 'Failed to unblock user');
                }
            } catch (error) {
                hideLoading();
                showAlert('error', 'Error', 'Failed to unblock user');
            }
        });
    });
});